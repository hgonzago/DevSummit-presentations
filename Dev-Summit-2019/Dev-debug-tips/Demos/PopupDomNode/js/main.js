require([
  ...
  "dojo/dom-construct",
  "dojo/domReady!"
], function (FeatureLayer, Map, WebScene, MapView, SceneView, Home, watchUtils, domConstruct) {
  ...
  sceneView.when(function () {
    ...
    popupView = new MapView({
      ...
      map: popupMap, // Update the popupmap to read popupMap
      ...
      watchUtils.when(sceneView, "stationary", function () {
        popupView.goTo({
          ...
          scale: sceneView.scale *
            2 *
            Math.max(sceneView.width / popupView.width, sceneView.height / popupView.height)
        });
      });
