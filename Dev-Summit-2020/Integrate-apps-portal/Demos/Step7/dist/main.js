define(["esri/WebMap", "esri/views/SceneView", "esri/portal/Portal", "esri/identity/OAuthInfo", "esri/identity/IdentityManager", "esri/layers/Layer", "esri/widgets/Expand", "esri/widgets/BasemapGallery", "esri/widgets/Search"], function (_WebMap, _SceneView, _Portal, _OAuthInfo, _IdentityManager, _Layer, _Expand, _BasemapGallery, _Search) {
  ...
  function applySharedTheme(portal) {
    if (portal && portal.portalProperties && portal.portalProperties.sharedTheme) {
      // get header background and text color and button background and text 
      var _portal$portalPropert = portal.portalProperties.sharedTheme,
          header = _portal$portalPropert.header,
          button = _portal$portalPropert.button;
      var headerDiv = document.getElementsByClassName("top-nav")[0];
      var signInButtons = document.getElementsByClassName("top-nav-link");
      var title = document.getElementById("title");
      var userName = document.getElementById("userName");

      if (header && headerDiv) {
        // apply bg color 
        if (header.background) headerDiv.style.backgroundColor = header.background;
        if (header.text && title) title.style.color = header.text;
      }

      if (button) {
        for (var i = 0; i < signInButtons.length; i++) {
          var buttonDiv = signInButtons[i];
          if (button.background) buttonDiv.style.backgroundColor = button.background;

          if (button.text) {
            buttonDiv.style.color = button.text;
            userName.style.color = button.text;
          }
