...
function applySharedTheme(portal) {
  if (portal && portal.portalProperties && portal.portalProperties.sharedTheme) {
    ...
    const {
      header,
      button,
    } = portal.portalProperties.sharedTheme;
    const headerDiv = document.getElementsByClassName("top-nav")[0];
    const signInButtons = document.getElementsByClassName("top-nav-link");
    ...
    if (button) {
      for (let i = 0; i < signInButtons.length; i++) {
        const buttonDiv = signInButtons[i];
